...
public class StickyNavLayout extends LinearLayout {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mInnerScrollView instanceof ScrollView) {
                    ...
                    if (c != null) {
                        Log.d("dengqu", "dispatchTouchEvent action =" + action + ",c.getTop() ==" + c.getTop() + ",isTopHidden =" + isTopHidden + ",dy=" + dy + ",isInControl =" + isInControl);
                    }
        ...
        boolean reslut = super.dispatchTouchEvent(ev);
        Log.d("dengqu", "dispatchTouchEvent reslut =" + reslut);
        return reslut;
    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(dy) > mTouchSlop) {
                    ...
                    if (mInnerScrollView instanceof ScrollView) {
                        ...
                        View c = rv.getChildAt(0);
                        if (c != null) {
                            Log.d("dengqu", "onInterceptTouchEvent action =" + action + ",c.getTop() ==" + c.getTop() + ",isTopHidden =" + isTopHidden + ",dy=" + dy);
                        }
    ...
    private void getCurrentScrollView() {
        ...
        if (!mInit) {
            mInit = true;
            init();
            if (mInnerScrollView != null && mInnerScrollView instanceof StickyRecyclerView) {
                ((StickyRecyclerView) mInnerScrollView).setListener(new StickyRecyclerView.DispatchTouchEventListener() {
                    @Override
                    public void dispatchTouchEvent(MotionEvent ev) {
                        ...
                        switch (action) {
                            case MotionEvent.ACTION_DOWN:
                                mDispatchTouchLastY = y;
                                break;
                            case MotionEvent.ACTION_MOVE:
                                try {
                                    float dy = y - mDispatchTouchLastY;
                                    RecyclerView rv = (RecyclerView) mInnerScrollView;
                                    View c = rv.getChildAt(0);
                                    if (c != null && c.getTop() == 0 && isTopHidden && dy > 0) {
                                        ev.setAction(MotionEvent.ACTION_CANCEL);
                                        MotionEvent ev2 = MotionEvent.obtain(ev);
                                        mInnerScrollView.dispatchTouchEvent(ev);
                                        ev2.setAction(MotionEvent.ACTION_DOWN);
                                        StickyNavLayout.this.dispatchTouchEvent(ev2);
                                    }
                                } catch (Exception e) {
                                    Log.e(TAG, "", e);
                                }
                ...
                ((StickyRecyclerView) mInnerScrollView).addOnScrollListener(new RecyclerView.OnScrollListener() {
                    @Override
                    public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                        Log.d("test", "onScrollStateChanged newState =" + newState + ",allDy =" + allDy);
                        if (newState == 0) {
                            if (mVelocityY < 0) {
                                double totalDistance = getSplineFlingDistance(mVelocityY);
                                int flingDistance = (int) (totalDistance * Math.signum(mVelocityY));
                                int remainDy = flingDistance - allDy;
                                Log.d("test", "onScrollStateChanged flingDistance =" + flingDistance + ",allDy =" + allDy + ",remainDy=" + remainDy);
                                StickyNavLayout.this.fling(remainDy);
                            }
                            allDy = 0;
                            mVelocityY = 0;
                        } else if (newState == 1) {
                    ...
                    public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                        Log.d("test", "onScrolled dy =" + dy);
                        allDy = allDy + dy;
                        super.onScrolled(recyclerView, dx, dy);
                    }
                });
                ((StickyRecyclerView) mInnerScrollView).setOnFlingListener(new RecyclerView.OnFlingListener() {
                    @Override
                    public boolean onFling(int velocityX, int velocityY) {
                        Log.d("test", "onFling velocityY =" + velocityY);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                float dy = y - mLastY;
                Log.d("dengqu", "onTouchEvent action =" + action + ",mDragging =" + mDragging + ",dy=" + dy + ",mTouchSlop =" + mTouchSlop);
